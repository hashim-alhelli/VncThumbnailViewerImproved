...
public class VncViewer extends java.applet.Applet
  ...
  public void run() {
    ...
    if (showControls) {
      ...
      vncContainer.add(buttonPanel);// this line adds the button panel to each frame
    }

    try {
		
		System.out.println("before connect and authentricate");
	...
	//System.out.println("vc == null is " + vc==null);
	validate();
	...
	buttonPanel.enableButtons();
      ...
      fatalError("00Network error: could not connect to server: " +
		 host + ":" + port, e);
	...
	//e.printStackTrace();
	System.out.println("0Network error: remote side closed connection");
	//flag = false;
	if (vc != null) {
	...
	fatalError("1Network Error: " + str, e);
      } else {
  ...
  void connectAndAuthenticate() throws Exception // this method throws the exception when connection cannot happen
  {
   ...
   //   vncFrame.show();
    ...
    showConnectionStatus("Cc to " + host + ", port " + port + "...");

	//System.out.println("before the bad guy");
	//System.out.printf("host is %s  port is %s \n",host,port);
    rfb = new RfbProto(host, port, this);//this is the bad guy
  ...
  void showConnectionStatus(String msg)
  {
	...
	vncContainer.remove(connStatusLabel);//if msg was null then this removes the label from the picture
    System.out.println("the layout is " + vncContainer.getLayout().toString());
   ...
   // System.out.println(host + " Status: " + msg); // DJC
    ...
    if (connStatusLabel == null) {
      ...
      connStatusLabel.setFont(new Font("Helvetica", Font.PLAIN, 10));
    } else {
		
		
      connStatusLabel.setText("Sstatus: " + msg);
    ...
    if (!vncContainer.isAncestorOf(connStatusLabel)) {
      ...
      vncContainer.add(connStatusLabel);//this adds the label to the image
    }
...
//synchronized
  synchronized public void disconnect() {
    ...
    if (inAnApplet) {
      ...
      buttonPanel.disableButtonsOnDisconnect();
     // System.exit(0);// this solves the exit issue where u cant exit the program once closed
    }
  }
  ...
  synchronized public void fatalError(String str) {
    System.out.println(str);
	  VncViewersList.tnViewer.removeViewer(this);//remove the view from the list and frame
	
	  this.disconnect();
	  
	  System.out.println("fatal error 1 " + VncViewersList.tnViewer.viewersList.size() + " " + VncViewersList.tnViewer.viewersList.seconnd_list.size());
	  vncFrame.revalidate();
	  vncFrame.repaint();
...
synchronized public void fatalError(String str, Exception e) {
 compcount = vncFrame.getComponentCount();
	  
	 
 this.disconnect();
 VncViewersList.tnViewer.removeViewer(this);
	  
 
 //VncViewersList.tnViewer.remove(this);
	VncThumbnailViewer.real_size--;
	  
	//VncViewersList.tnViewer.remove(this);
	 
	  System.out.println("fatal error 2 " + VncViewersList.tnViewer.viewersList.size() + " " + VncViewersList.tnViewer.viewersList.seconnd_list.size());
	  //
	  vncFrame.repaint();
	  vncFrame.validate();
	//  vncFrame.pack(); // this is the pack that changes size of the window
